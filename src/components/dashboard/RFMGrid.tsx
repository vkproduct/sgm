...
export function RFMGrid({ customers }: RFMGridProps) {
    ...
    const getSegmentColor = (segment: string) => {
        switch (segment) {
            case 'Чемпионы': return 'bg-emerald-500 text-white';
            case 'Лояльные клиенты': return 'bg-emerald-400 text-white';
            case 'Потенциально лояльные': return 'bg-emerald-300 text-emerald-900';
            case 'Перспективные': return 'bg-yellow-300 text-yellow-900';
            case 'Требуют внимания': return 'bg-yellow-400 text-yellow-900';
            case 'В зоне риска': return 'bg-orange-400 text-white';
            case 'Нельзя потерять': return 'bg-orange-300 text-orange-900';
            case 'Спящие': return 'bg-red-300 text-red-900';
            case 'Потерянные': return 'bg-red-500 text-white';
    ...
    const getSegmentDescription = (segment: string): string => {
        const descriptions: Record<string, string> = {
            'Чемпионы': 'Ваши лучшие клиенты! Покупают часто и недавно, тратят больше всех. Награждайте их эксклюзивными предложениями и ранним доступом к новинкам.',
            'Лояльные клиенты': 'Постоянные покупатели с высокой частотой покупок. Поощряйте их программами лояльности и персональными скидками.',
            'Потенциально лояльные': 'Недавние покупатели с хорошим потенциалом. Предложите членство в программе лояльности и персонализированные рекомендации.',
            'Перспективные': 'Новые клиенты, которые недавно совершили покупку. Вовлекайте их с помощью онбординга и специальных предложений для новичков.',
            'Требуют внимания': 'Клиенты с умеренной активностью. Отправьте персонализированные предложения на основе их истории покупок.',
            'В зоне риска': 'Раньше покупали часто, но давно не возвращались. Срочно отправьте win-back кампанию с привлекательным предложением.',
            'Нельзя потерять': 'Ценные клиенты, которые перестали покупать. Свяжитесь лично, узнайте причину и предложите VIP-условия возврата.',
            'Спящие': 'Давно не покупали, низкая частота. Попробуйте реактивировать через опрос с бонусом или напоминание о бренде.',
            'Потерянные': 'Потерянные клиенты. Последняя попытка - агрессивная скидка или новое ценностное предложение. Если не сработает - исключите из активных кампаний.',
        };
